...
def get_client_auth(request: Request, google_info = Depends(get_google_info)):
    flow = google_auth_oauthlib.flow.Flow.from_client_config(google_info,
        scopes=SCOPES)
...
@router.get('/callback')
def callback(request: Request, google_info = Depends(get_google_info), settings: Settings = Depends(get_settings)):
    flow = google_auth_oauthlib.flow.Flow.from_client_config(google_info,
                                                             scopes=SCOPES)
    flow.redirect_uri = request.url_for("callback")
    ...
    id_info = id_token.verify_oauth2_token(creds._id_token, req, google_info['web']['client_id'])

    if 'hd' not in id_info or id_info['hd'] != settings.top_domain:
        raise HTTPException(status_code=403, detail=f"Not an authorized domain {id_info.get('hd', '')}, {settings.top_domain}")
